## netcontext.R - Using ggnetwork library to "project" gene models onto their network context
##
## by Artem Sokolov
...
opts <- list(
    ...
    make_option(
        ...
        help="(Optional) Output filename. Default: out.pdf" ) )
...
network.context <- function( w, g, seed )
{
    ...
    ggplot( gs, aes(x = x, y = y, xend = xend, yend = yend ) ) +
        ...
        geom_nodes( aes( fill = Weight, size=Type ), color="gray", shape=21 ) +
